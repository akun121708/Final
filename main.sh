...
function LOGO() {
 ...
 ───│    $Green├─┤│ │ │ │ │└─┐│  ├┬┘│├─┘ │   │  │ │ ├┤ $NC   │───
 ───│    $Green┴ ┴└─┘ ┴ └─┘└─┘└─┘┴└─┴┴   ┴   ┴─┘┴ ┴ └─┘$NC   │───
    │    ${YELLOW}Copyright${FONT} (C)$GRAY https://t.me/MahirMandala$NC    │
    └───────────────────────────────────────────────┘
...
MYIP=$(curl -sS ipv4.icanhazip.com)
    IZIN=$(curl -sS https://raw.githubusercontent.com/akun121708/izinvps/ipuk/ip | awk '{print $4}' | grep $MYIP)
    if [ "$MYIP" = "$IZIN" ]; then
        TIMEDATE
    else
...
MMPROJECT() {
    curl -sS https://raw.githubusercontent.com/akun121708/izinvps/ipuk/ip >/root/tmp
...
apete_eee() {
    ...
    if [ -f /home/needupdate ]; then
        ...
        green "Contact telegram https://t.me/MahirMandala"
        sleep 8
        exit 0
    fi
}
...
REPO="https://raw.githubusercontent.com/akun121708/abc/ipuk/"
CDNF="https://raw.githubusercontent.com/akun121708/abc/ipuk"
...
function finish(){
    USRSC=$(curl -sS https://raw.githubusercontent.com/akun121708/izinvps/ipuk/ip | grep $MYIP | awk '{print $2}')
    EXPSC=$(curl -sS https://raw.githubusercontent.com/akun121708/izinvps/ipuk/ip | grep $MYIP | awk '{print $3}')
